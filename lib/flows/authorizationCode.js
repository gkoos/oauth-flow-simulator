...
export function handleAuthorize(req, res, { isValidRedirectUri, authCodes, clients }) {
  ...
  if (!username) {
    // --- PKCE: Store code_challenge and code_challenge_method in session if present ---
    ...
    if (code_challenge) {
      if (req.session) {
        ...
        if (code_challenge_method) req.session.code_challenge_method = code_challenge_method;
        else delete req.session.code_challenge_method;
      }
    } else if (req.session) {
  ...
  // --- PKCE: If session has code_challenge, associate with authCodes[code] ---
  if (req.session && req.session.code_challenge) {
    authCodes[code].pkce = {
      code_challenge: req.session.code_challenge
    };
    // Only store code_challenge_method if present
    if (req.session.code_challenge_method) {
      authCodes[code].pkce.code_challenge_method = req.session.code_challenge_method;
    }
