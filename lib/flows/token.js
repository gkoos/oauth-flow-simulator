...
export function handleToken(req, res, { getClient, isValidRedirectUri, authCodes, users, tokens, refreshTokens }) {
  ...
  if (grant_type === 'authorization_code') {
    ...
    if (!meta || meta.username == null) {
      console.debug('DEBUG: Invalid refresh token - not found or missing username', { refresh_token, meta });
      return res.status(400).json({ error: 'invalid_grant', error_description: 'Invalid refresh token' });
    }
    // If token exists but client does not match, return invalid_grant (400)
    if (meta.clientId !== client.clientId) {
      console.debug('DEBUG: Refresh token client mismatch', { refresh_token, metaClientId: meta.clientId, clientClientId: client.clientId });
      return res.status(400).json({ error: 'invalid_grant', error_description: 'Invalid refresh token' });
    }
    if (meta.expiresAt && meta.expiresAt < Date.now()) {
      console.debug('DEBUG: Refresh token expired', { refresh_token, expiresAt: meta.expiresAt, now: Date.now() });
...
export function handleRevoke(req, res, { getClient, tokens, refreshTokens }) {
  // Requires client authentication (should be enforced by middleware)
  ...
  if (!revoked && (token_type_hint === 'refresh_token' || !token_type_hint)) {
    if (refreshTokens[token]) {
      delete refreshTokens[token];
      revoked = true;
    }
  }
  // Per RFC 7009, always return 200 OK, even if token was invalid or already revoked
