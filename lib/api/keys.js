...
router.get("/sim/keys", (req, res) => {
  res.json({
    activeKid: signingKeys.activeKid,
  ...
  try {
    if (kty === "RSA" || alg === "RS256") {
      ...
      try {
        ...
        const pubJwk = publicKey.export({ format: "jwk" });
        const privJwk = privateKey.export({ format: "jwk" });
        if (!pubJwk.crv || !pubJwk.x || !pubJwk.y || !privJwk.d) {
          return res.status(500).json({ error: "EC key export failed or curve not supported. Check Node.js version and curve name." });
        }
        keyObj = {
          kid: newKid,
          kty: "EC",
        ...
        return res.status(500).json({ error: `EC key generation failed: ${err.message}` });
      }
    } else {
