...
export function createClient(req, res) {
  const { clientId, clientSecret, redirectUris, scopes, refreshTokenLifetime } = req.body;
  // Uniqueness check only; OpenAPI validator handles required/type
  if (clients.find(c => c.clientId === clientId)) {
    return res.status(409).json({ error: 'Client already exists' });
  }
  const client = { clientId, clientSecret, redirectUris, scopes: scopes || [], refreshTokenLifetime };
  clients.push(client);
...
export function updateClient(req, res) {
  const client = clients.find(c => c.clientId === req.params.clientId);
  if (!client) return res.status(404).json({ error: 'Client not found' });
  const { clientSecret, redirectUris, scopes, refreshTokenLifetime } = req.body;
  ...
  if (refreshTokenLifetime !== undefined) client.refreshTokenLifetime = refreshTokenLifetime;
  res.json(client);
}
